...
export async function POST(req: Request) {
  ...
  if (file.size > MAX_BYTES) return NextResponse.json({ error: "El archivo supera el límite de 5 MB." }, { status: 413 });
  if (!/\.xlsx$/i.test(file.name)) return NextResponse.json({ error: "Formato no soportado. Sube un archivo .xlsx." }, { status: 415 });
