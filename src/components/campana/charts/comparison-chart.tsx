...
import { CHANNELS, CHANNEL_ORDER } from "@/lib/campana/constants";
import type { ChannelKey } from "@/lib/campana/types";
import { formatCOP, formatCOPCompact, formatNumber } from "@/lib/campana/format";
...
export function ComparisonChart({ data, mode = "currency" }: ComparisonChartProps) {
  ...
  const hasRealData = data.some((d) =>
    CHANNEL_ORDER.some((ch) => (d[`${ch}_real` as keyof ComparisonDatum] as number) > 0),
  );
